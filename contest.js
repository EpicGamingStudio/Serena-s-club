...
function startTimer(endTime, secret) {
  currentSecret = secret; // store secret for ranking
  ...
  function tick() {
    ...
    if (remainingMs > 0) {
      ...
      countdownEl.textContent = `Time left: ${secs}s`;
...
async function getUsername(uid) {
  try {
    ...
    return currentUser.email; // fallback
  } catch {
...
deleteGuessesBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    for (const doc of snapshot.docs) {
      await guessesRef.doc(doc.id).delete();
    }
...
async function showRankingAfterSecret() {
  ...
  sorted.forEach((g,index) => {
    ...
    if(index <= 4) div.classList.add('winner'); // top 5 highlighted
